...
$calculator_text          = '';
$shipping_label           = 'Verteilung durch den SVB';
...
foreach ( $shipping_classes as $shipping_class ) {
	if ( 192 === $shipping_class->term_id ) {
    	$shipping_label  = $shipping_class->name;
    }
}
$new_method = (object) [
	'id' => 'flat_rate:6',
		...
		<?php if ( $available_methods ) :
			foreach ($available_methods as $method_key => $method ) {
				if (  has_password_protected_product() && 'flat_rate:6' !== $method_key ) {
					$available_methods['flat_rate:6'] = $new_method;
			...
			if ( has_password_protected_product() ) {
				...
				foreach ( $available_methods as $shipping_method_key => $shipping_method ) {
	       
		            if ( 6 === $shipping_method->get_instance_id() ) {
		                unset($available_methods[ $shipping_method_key ]);
		            }
	        	}
	        }
	        */
				...
				<?php foreach ( $available_methods as $method ) : ?>
					<li>
						...
						if (  'flat_rate:6' === $method->id ) {
							printf( '<label for="shipping_method_%1$s_%2$s">%3$s</label>', $index, esc_attr( sanitize_title( $method->id ) ), $method->label ); // WPCS: XSS ok.
						} else {
